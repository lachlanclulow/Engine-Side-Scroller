...
public abstract class BaseWorld implements traverseConstants {
	...
	public abstract void update(double dirX, double dirY, boolean jump, int delta);
	...
	public static boolean tileTraversable(BoxCollider col, 
		...
		if (direction == XDIRECTION) {
			...
			if (col.getX() - col.getWidth()/2.0 + travelDistance <= 1 || 
					col.getX() + col.getWidth()/2.0 + travelDistance + 1 >= 
					map.getWidth() * map.getTileWidth())
				return false;
		...
		// for y direction
			...
			// Check y coordinate is on the map
			if (col.getY() - col.getHeight()/2.0 + travelDistance <= 1 ||
					col.getY() + col.getHeight()/2.0 + travelDistance >=
					map.getHeight() * map.getTileHeight())
				return false;
